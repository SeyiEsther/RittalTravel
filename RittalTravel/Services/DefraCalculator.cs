...
{
    // DEFRA 2025 GHG Conversion Factors - Table 5: Business Travel
    ...
    // All 15 transport modes - factors are kgCO2e per passenger-km
    ...
    {
        if (!Factors.TryGetValue(mode, out double factor))
            return 0.0;
    ...
    public static string GetDistanceMethodology(string mode)
    {
        ...
        if (mode.StartsWith("Car", StringComparison.OrdinalIgnoreCase))
            return "Google Maps Distance Matrix";
        if (mode.StartsWith("Bus", StringComparison.OrdinalIgnoreCase) ||
            mode.StartsWith("Coach", StringComparison.OrdinalIgnoreCase))
            return "Google Maps Distance Matrix";
        if (mode.StartsWith("Taxi", StringComparison.OrdinalIgnoreCase))
            return "Google Maps Distance Matrix";
        if (mode.StartsWith("Ferry", StringComparison.OrdinalIgnoreCase))
            return "Haversine Great Circle";
        return "Google Maps Distance Matrix";
    }
