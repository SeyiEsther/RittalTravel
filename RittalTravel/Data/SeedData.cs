...
public static class SeedData
{
    ...
    private static async Task SeedTrips(RittalTravelContext db, ILogger logger)
    {
        ...
        // Pre-calculated distances using Haversine (fallback since no API key at seed time)
        var distances = new Dictionary<string, double>
        {
            { "London-Frankfurt",  898.0  },
            { "Birmingham-Brussels", 475.0 },
            { "Manchester-London",  263.0  },
            { "London-New York",   5539.0 },
            { "Bristol-Birmingham",  93.0  },
            { "Sheffield-Hellaby",    8.0  },
            { "London-Edinburgh",   534.0  },
            { "Manchester-Leeds",    65.0  },
        };
        ...
        foreach (var s in seedData)
        {
            ...
            string formula = DefraCalculator.GetFormula(s.Mode, s.Class, dist, factor, s.Passengers, kg);
            string methodology = DefraCalculator.GetDistanceMethodology(s.Mode);
            ...
            trips.Add(new Trip
            {
                ...
                DefraFactorYear      = "DEFRA 2025",
                LoggedBy             = "admin@rittal.co.uk",
